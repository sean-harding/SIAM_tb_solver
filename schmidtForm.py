...
def schmidt(H,DM,impuritySize):
    ...
    U_imp = sp.linalg.block_diag(u_imp[1],sp.eye(len(H)-impuritySize))
    unitary = sp.matrix(U_imp)
    H2 = U_imp.transpose()*H*U_imp
    ...
    sort = sp.roll(sort,-1*empty)       #Reorder to put impurity modes in the middle
    #u2 = u2[:,sort]
    U = sp.linalg.block_diag(sp.eye(impuritySize),u2)
    U = sp.matrix(U)
    H2 = U.transpose()*H_initial*U
    unitary = unitary*U
